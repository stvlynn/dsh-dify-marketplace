 ...
 */
export function parseUniqueIdentifier(value) {
 ...
 */
export function loaderEntryId(pluginId) {
 ...
 */
export function toolName(org, plugin, tool) {
    const raw = `dify__${org}__${plugin}__${tool}`;
    const normalized = raw.replace(TOOL_NAME_ALLOWED, '_');
    if (normalized === raw && raw.length <= MAX_TOOL_NAME_LENGTH)
        return raw;
 ...
 */
export function stateFileName(pluginId) {
    const { org, name } = parsePluginId(pluginId);
