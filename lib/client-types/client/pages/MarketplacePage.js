...
export function MarketplacePage({ t }) {
    ...
    return (_jsxs("div", { className: styles.root, children: [_jsx("p", { children: t('subtitle') }), status !== undefined && (_jsxs("div", { className: styles.banner, children: [_jsxs("span", { className: styles.status, children: [_jsx(StateDot, { state: status.marketplace.reachable ? 'done' : 'error' }), status.marketplace.reachable ? t('marketplaceUp') : t('marketplaceDown')] }), _jsxs("span", { className: styles.status, children: [_jsx(StateDot, { state: status.daemon.configured ? (status.daemon.reachable ? 'done' : 'error') : 'warning' }), !status.daemon.configured ? t('daemonMissing') : status.daemon.reachable ? t('daemonUp') : t('daemonDown')] })] })), _jsxs("div", { className: styles.toolbar, children: [_jsx("div", { className: styles.search, children: _jsx(Input, { value: query, placeholder: t('searchPlaceholder'), onChange: event => setQuery(event.target.value), onKeyDown: (event) => { if (event.key === 'Enter')
