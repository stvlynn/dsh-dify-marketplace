...
export interface MarketplacePluginResource {
  ...
  permission: {
    ...
    app?: { enabled?: boolean }
    storage?: { enabled?: boolean, size?: number }
  }
}
...
export interface DifyToolParameter {
  ...
  min?: number | null
  max?: number | null
  options?: { value: string, label: DifyI18nObject }[] | null
  scope?: string | null
...
export interface DifyModelProvider {
  ...
  provider_credential_schema?: { credential_form_schemas?: unknown[] } | null
...
export interface MarketplacePluginSummary {
  ...
  badges: string[] | null
  tags: { name: string }[]
  verification: { authorized_category: MarketplaceAuthorizedCategory }
...
export interface MarketplacePluginDetail extends MarketplacePluginSummary {
  introduction: string
  readme_meta?: { available_languages?: string[] } | null
...
export const MARKETPLACE_SORT_FIELDS = ['install_count', 'version_updated_at', 'created_at'] as const
