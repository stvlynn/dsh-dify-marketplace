...
import { TRIGGER_OPERATIONS } from '../../host/domain/capability.ts'
import { toolName } from '../../shared/identifier.ts'
import { defineDifyTool } from '../define-dify-tool.ts'
import type { CapabilityAdapter } from '../deps.ts'

/** Trigger adapter. */
export const registerTriggerAdapter: CapabilityAdapter = (ctx, config, deps) => {
  const names: string[] = []
