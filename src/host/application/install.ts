...
import type {
  ...
  BridgeInstallTaskResponse,
  BridgeInstalledPlugin,
  BridgeInstalledResponse,
  BridgeUninstallResponse,
} from '../../shared/contracts/bridge.ts'
import { parsePluginId, parseUniqueIdentifier } from '../../shared/identifier.ts'
import { localized } from '../../shared/localized.ts'
import { DifyMarketplaceError } from '../domain/errors.ts'
import { snapshotFromDetail } from '../domain/snapshot.ts'
...
const tasks = new Map<string, { uniqueIdentifier: string, pluginId: string }>()
...
export class InstallService {
   ...
   */
  async install(uniqueIdentifier: string): Promise<BridgeInstallResponse> {
    ...
    const decoded = await this.deps.daemon.uploadPackage(
      downloaded.bytes,
      `${identity.name}.difypkg`,
      this.deps.config.verifySignature,
    )
    const started = await this.deps.daemon.installFromIdentifiers(
   ...
   */
  async installTask(taskId: string): Promise<BridgeInstallTaskResponse> {
  ...
  async installed(): Promise<BridgeInstalledResponse> {
    const records = await this.deps.state.list()
    const plugins: BridgeInstalledPlugin[] = []
   ...
   */
  async uninstall(pluginId: string): Promise<BridgeUninstallResponse> {
  ...
  ): Promise<BridgeCredentialsResponse> {
    const record = await this.deps.state.get(pluginId)
    if (record === undefined) {
      throw new DifyMarketplaceError('plugin_not_installed', `${pluginId} is not installed`)
    }
  ...
  private async finishInstall(uniqueIdentifier: string): Promise<InstalledPluginState> {
    const identity = parseUniqueIdentifier(uniqueIdentifier)
    ...
    if (installation === undefined) {
      throw new DifyMarketplaceError(
        'install_failed',
        `daemon did not list ${identity.pluginId} after a successful install task`,
      )
    }
    let endpointHookId: string | undefined
    ...
    const state: InstalledPluginState = {
      ...
      label: Object.fromEntries(
        Object.entries(detail.label).filter((entry): entry is [string, string] => typeof entry[1] === 'string'),
      ),
      icon: detail.icon,
      toolNames: [],
  ...
  private async toInstalled(record: InstalledPluginState): Promise<BridgeInstalledPlugin> {
...
export function installLabel(record: InstalledPluginState): string {
